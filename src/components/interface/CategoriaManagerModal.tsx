...
export function CategoriaManagerModal({
  ...
  const [periodoaplicacion, setPeriodoaplicacion] = useState('Mensual');
  ...
  const resetForm = () => {
    setEditingId(null);
    setNombre('');
    setEstadolimite(false);
    setImporte('');
    setMoneda('ARS');
    setPeriodoaplicacion('Mensual');
    setErrorMsg('');
  };
  ...
  const handleStartEdit = (cat: CategoriaItem) => {
    ...
    setEstadolimite(cat.estadolimite || false);
    setImporte(cat.importe !== null ? String(cat.importe) : '');
    setMoneda(cat.moneda || 'ARS');
    setPeriodoaplicacion(cat.periodoaplicacion || 'Mensual');
    setErrorMsg('');
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-3 p-3 bg-slate-950/60 rounded-xl border border-slate-800 shrink-0">
          ...
          {estadolimite && isAdmin && (
            <div className="grid grid-cols-3 gap-2 pt-2 animate-in fade-in duration-150">
              ...
              <div>
                <label className="text-[10px] text-slate-400">Periodo</label>
                <select
                  value={periodoaplicacion}
                  onChange={(e) => setPeriodoaplicacion(e.target.value)}
                  className="w-full h-8 px-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-semibold text-slate-200"
                >
